...
# --- TARGET 1: DEER LOGIC (Tolerance-based) ---
DEER_R = 71
DEER_G = 73
DEER_B = 81
DEER_TOLERANCE = 40
...
def get_camera_rgb(camera, interval, state):
    ...
    if state["camera_interval"] >= interval:
        ...
        # --- ROI FIX: Define boundaries to ignore the floor/ceiling ---
        start_y = int(height * 0.3)  # Start scanning 30% down from the top
        end_y = int(height * 0.7)    # Stop scanning 30% up from the bottom
        pixels_scanned = 0
        
        for x in range(width):
            for y in range(start_y, end_y): # Only loop through the middle band!
        ...
        return (
            int(r / pixels_scanned),
            int(g / pixels_scanned),
            int(b / pixels_scanned),
        )
    else:
...
def run_robot(robot):
    ...
    print("Targets: Deer, Red Block, Green Block, Blue Block")
    print("=========================================")
    ...
    while robot.step(timestep) != -1:
        ...
        if not (red == 0 and green == 0 and blue == 0):
            ...
            # --- CHECK 1: Is it the Deer? (Tolerance Math) ---
            if (abs(red - DEER_R) < DEER_TOLERANCE and 
                abs(green - DEER_G) < DEER_TOLERANCE and 
                abs(blue - DEER_B) < DEER_TOLERANCE):
                detected_object = "deer"
           ...
           # --- STATE MACHINE: Encounter & Capture ---
            if detected_object and detected_object not in encountered:
                ...
                # --- NEW LOGIC: Only capture if it is the deer ---
                if detected_object == "deer":
